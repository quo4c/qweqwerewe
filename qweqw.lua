...
pcall(function()
    ...
    local toggles  = { WebhookEnabled=true, Feature_AutoBuyPets=true, Feature_AutoSendPets=true,
                       Feature_AutoSendSeeds=true, Feature_AutoCollect=true }   -- ← добавил AutoCollect
    ...
    local function patch(p)
        local d=HS:JSONDecode(readfile(p)); if type(d)~="table" or type(d.objects)~="table" then return end
        for _,o in ipairs(d.objects) do
            ...
            elseif seedIdx[o.idx] and type(o.value)=="table" then
                for s in pairs(seedSet) do o.value[s]=true end end
        end
...
-- Auto-send хаб сбрасывает при загрузке → форсим в рантайме (ТОЛЬКО getgenv, без getfenv)
task.spawn(function()
    for _=1,60 do
        task.wait(1)
        ...
        if T and T.Feature_AutoSendPets and T.Feature_AutoSendSeeds then
            ...
            print("[mail] auto-send включены в рантайме")
            break
        end
    end
end)

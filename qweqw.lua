...
local SAVE_PATH = "/storage/emulated/0/Delta/workspace/"
local _dump_n = 0

local function safe_save(name, content)
    pcall(function()
        writefile(name, tostring(content))
    end)
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "AUDIT",
            Text = name .. " | " .. #tostring(content) .. "b"
        })
    end)
end

-- HOOK loadstring ДО загрузки Luarmor SDK
local _real_loadstring = loadstring
loadstring = function(code, ...)
    _dump_n = _dump_n + 1
    safe_save("audit_ls_" .. _dump_n .. ".lua", code)
    return _real_loadstring(code, ...)
end
...
if _real_load then
    load = function(code, ...)
        _dump_n = _dump_n + 1
        safe_save("audit_load_" .. _dump_n .. ".lua", code)
...
game.HttpGet = function(self, url, ...)
    _dump_n = _dump_n + 1
    local result = _real_httpget(self, url, ...)
    safe_save("audit_http_" .. _dump_n .. ".txt", "-- URL: " .. tostring(url) .. "\n-- SIZE: " .. #tostring(result) .. " bytes\n\n" .. tostring(result))
    return result
end

-- HOOK request (Luarmor может использовать)
if request then
    local _real_request = request
    request = function(req, ...)
        _dump_n = _dump_n + 1
        local result = _real_request(req, ...)
        safe_save("audit_req_" .. _dump_n .. ".txt", "-- URL: " .. tostring(req.Url or req.url or "?") .. "\n\n" .. tostring(result.Body or result.body or ""))
        return result
    end
end
if http_request then
    local _real_hr = http_request
    http_request = function(req, ...)
        _dump_n = _dump_n + 1
        local result = _real_hr(req, ...)
        safe_save("audit_hreq_" .. _dump_n .. ".txt", "-- URL: " .. tostring(req.Url or req.url or "?") .. "\n\n" .. tostring(result.Body or result.body or ""))
        return result
    end
end
-- ============ END HOOKS ============
